...
public class CoursesController : Controller
{
    ...
    public async Task<IActionResult> Index(CancellationToken ct = default)
    {
        ...
        var list = await _db.Courses.Include(c => c.Category).Where(c => c.InstructorId == userId).OrderByDescending(c => c.CreatedAt).AsNoTracking().ToListAsync(ct);
        return View(list);
    }
    ...
    {
        var userId = GetUserId();
        var categories = await _db.Categories.OrderBy(c => c.SortOrder).AsNoTracking().ToListAsync(ct);
        ViewBag.Categories = new SelectList(categories, "Id", "Name");
        if (!id.HasValue)
        {
            return View(new Course { IsPublished = false });
        }
        var course = await _db.Courses.Include(c => c.CurriculumItems.OrderBy(x => x.SortOrder)).FirstOrDefaultAsync(c => c.Id == id && c.InstructorId == userId, ct);
    ...
    public async Task<IActionResult> Edit(Course model, CancellationToken ct = default)
    {
        ...
        var course = await _db.Courses.FirstOrDefaultAsync(c => c.Id == model.Id && c.InstructorId == userId, ct);
        if (course == null) return NotFound();
